...
function tableActionOptions(data, type, row, meta)
{
	...
	if ("" !== data)
	{
		...
		if ("" !== splitted[0])
		{
			retVal += '<button type="button" class="sdb-control-button" onclick="sendGAEvent(\'action\', \'mqaevent-download-data\', \'Zip=' + humanIdFromRow(row) + '\'); location.href=\'' + getUrlpath() + splitted[0] + '\';" title="Download Data Archive">' +
					'<i class="fas fa-download" style="color: blue;"> </i></button>';
		}
		if ("" !== splitted[1])
		{
			retVal += '<button type="button" class="sdb-control-button" onclick="sendGAEvent(\'action\', \'mqaevent-download-results\', \'Zip=' + humanIdFromRow(row) + '\'); location.href=\'' + getUrlpath() + splitted[1] + '\';" title="Download Results Archive">' +
