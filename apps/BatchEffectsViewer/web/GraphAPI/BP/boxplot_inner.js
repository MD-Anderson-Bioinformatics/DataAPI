							...
							theOutlierPoints, theColor, theElementHistogramData, theHistCount)
	{
		...
		if(null!==theElementHistogramData)
		{
			...
			leftLineData.push(
				{ 
					"x": histogramScaleLeftX.range()[0],  
					"y": histogramScaleY(theElementHistogramData["x" + 0])
				});
			rightLineData.push(
				{
						"x": histogramScaleLeftX.range()[0], 
			...
			for(var i=0;i<theHistCount;i++)
			{
				...
				var pointObjLeft = 
					{ 
						"x": histogramScaleLeftX(theElementHistogramData["y" + i]) , 
						"y": histogramScaleY(theElementHistogramData["x" + i])
					};
				leftLineData.push(pointObjLeft);
				var pointObjRight = 
					{ 
						"x": histogramScaleRightX(theElementHistogramData["y" + i]) , 
						"y": histogramScaleY(theElementHistogramData["x" + i])
					};
			...
			leftLineData.push(
				{ 
					"x": histogramScaleLeftX.range()[0], 
					"y": histogramScaleY(theElementHistogramData["x" + (theHistCount-1)])
				});
			leftLineData.push(
				{
						"x": histogramScaleLeftX.range()[0], 
