			...
			return new Promise((resolveTop, reject) => 
			{
				...
				fr.onload = function ()
				{
					...
					JSZip.loadAsync(data).then(function(theZip)
					{
						...
						theZip.file("index.json").async("text").then(function(theText)
						{
							resolveTop(JSON.parse(theText));
						});
					}, 
					function (e)
					{
		...
		if (notUN(thisDA.fileZip))
		{
			return new Promise((resolveTop, reject) => 
			{
				...
				fr.onload = function ()
				{
					...
					{
						//console.log(theZip);
						//console.log("getDataFile Looking for " + theTextFile);
						theZip.file(theTextFile).async("text").then(function(theText)
						{
							resolveTop(theText);
						});
					}, 
